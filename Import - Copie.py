...
keysToRemove = ["user_02_s", "user_03_s", "user_05_s", "user_05_d", "user_03_d", "detected_in_sprint", "user_06_s", "user_14_s", "user_12_s", "user_11_s", "user_07_s", "user_08_s", "user_03", 
"user_02", "user_01", "user_01_s", "user_09_s", "pending_acceptance_tests", "passed_acceptance_tests", "user_04_s", "team_id", 
"archive_status", "application_id", "rank", "cover_status", "failed_acceptance_tests", "kanban_status_id", "estimated", 
"kanban_parent_status_HP AGM ID", "kanban_parent_status_id", "blocked", "actual", "sprint_id", "kan_parent_duration", "invested", "HP AGM ID", "kan_status_duration", "dev_comments", "estimate", "remaining", "num_of_defects",
"num_of_user_stories", "status_duration", "wsjf_job_size", "user_10", "initial_estimate", "wsjf_risk", "wsjf_user_impact", "user_13", 
"wsjf_cod", "applications", "agg_story_points", "wsjf_time_effort", "owner", 
"wsjf", "agg_bli_story_points", "defect_priority", "defect_status", "user_04_d", 
"user_06_d", "user_07_d", "user_08_d", "user_09_d", "user_11_d", "detected_in_rel", "severity", "fixed_on_date", "detection_date", "closing_date", 
"user_01_d", "detected_by", "user_02_d", "done_status_date"]
            ...
            if key == "author":
                ...
                if obj[key] == "":
                    ...
                    if usernameids.has_key(keyString):
                        ...
                        print "Adding user - reporter" 
                    else:
# Initial code                        obj["reporter"] = obj[key]
                        obj["reporter"] = projectAdmin
                        print "Project Admin set as reporter"
            ...
            if key == "assigned_to":
                ...
                if obj[key] == "":
                    ...
                    if usernameids.has_key(keyString):
                        ...
                        print "Adding user - assignee"
                    else:
# Initial code                        obj[new_key] = obj[key]
                        obj[new_key] = projectAdmin
                        print "Project Admin set as assignee"
                        namesNotFound.append(obj[key])
                    del obj[key]
            if key == "num_of_tasks":
                if obj[key] is not None:
...
def loginFunction():
    ...
    payload = 'client_id=XXXXXXXX&client_secret=XXXXXXXXXX&grant_type=client_credentials'
...
def backlog_itemsFunction(access_token):
    numberOfPasses = 0
    print 'Pass number: ' + str(numberOfPasses)
    offset = 0
    currentResults = 0
    ...
    while totalResults > currentResults: 
        ...
        print 'Backlog Items Pass number: ' + str(numberOfPasses) + 'Total results: ' + str(totalResults)
...
def featuresFunction(access_token, themeIds, featureIds):
    numberOfPasses = 0
    print 'Features Pass number: ' + str(numberOfPasses)
    offset = 0
    currentResults = 0
    bearer = 'bearer ' + access_token
    ...
    #Debug    print url2
        ...
        while totalResults > currentResults: 
            numberOfPasses = numberOfPasses + 1
            print 'Features Pass number: ' + str(numberOfPasses) + 'Total results: ' + str(totalResults)
            payload = "?query=" + urllib.quote_plus(queryString) + '&offset=' + str(offset)
            url2 = url + payload
    #Debug        print url2
            r = requests.get(url2, headers=headers)
...
def themesFunction(access_token, applicationId):
    themesToKeep = list()
    numberOfPasses = 0
    print 'Theme Pass number: ' + str(numberOfPasses)
    offset = 0
    currentResults = 0
    bearer = 'bearer ' + access_token
    ...
    while totalResults > currentResults: 
        offset = offset + 100
        numberOfPasses = numberOfPasses + 1
        print 'Themes Pass number: ' + str(numberOfPasses) + 'Total results: ' + str(totalResults)
...
def tasksFunction(access_token, listOfBacklogItemIds):
    numberOfPasses = 0
    print 'Tasks Pass number: ' + str(numberOfPasses)
    offset = 0
    currentResults = 0
    bearer = 'bearer ' + access_token
    ...
    while totalResults > currentResults: 
        offset = offset + 100
        numberOfPasses = numberOfPasses + 1
        print 'Tasks Pass number: ' + str(numberOfPasses) + 'Total results: ' + str(totalResults)
...
print 'Features data length: ' + str(len(features))

for backlogItem in backlogData:
    jiraifyKeys(backlogItem, features)
print 'Backlog data length: ' + str(len(backlogData))
...
print 'Tasks data length: ' + str(len(tasks))

issueData = themes + features + backlogData + tasks
print str(len(issueData))
